...
class MeshTextureRender:
    def __init__(
        ...
        y_offset: Annotated[float, "Y offset for the camera"] = 0.0,
    ) -> None:
    ...
    def load_mesh(self) -> Meshes:
        try:
            mesh = load_objs_as_meshes(
                [self.obj_path], device=self.device, load_textures=True
            )
    ...
    def renderer(self) -> MeshRenderer:
        ...
        raster_settings = RasterizationSettings(
            image_size=self.image_size, blur_radius=0.0, faces_per_pixel=1
        )
    ...
    def render_360_views(
        self, num_views: int = 10, output_prefix: str = "render"
    ) -> str:
        mesh = self.load_mesh()
